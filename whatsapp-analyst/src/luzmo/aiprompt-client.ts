...
export interface StreamAIPromptParams {
  question: string;
  luzmoKey: string;
  luzmoToken: string;
  luzmoHost: string;
...
export async function* streamAIPrompt(
  ...
  const body = {
    ...
    properties: {
      ...
      locale_id: params.localeId ?? 'en',
      timezone_id: params.timezoneId ?? 'UTC',
      ...(params.conversationId
        ? { conversation_id: params.conversationId }
        : {}),
      input: [
        { type: 'text', text: params.question },
        { type: 'dataset', id: params.datasetId },
      ],
    },
  };
...
export interface CollectedAIPromptResult {
  text: string;
  conversationId?: string;
  item?: ItemConfig;
  error?: string;
...
export async function collectAIPrompt(
  params: StreamAIPromptParams
): Promise<CollectedAIPromptResult> {
  let text = '';
  let conversationId = params.conversationId;
  let item: ItemConfig | undefined;
  let error: string | undefined;

  for await (const msg of streamAIPrompt(params)) {
    if (isStartMessage(msg)) {
      conversationId = msg.conversationId;
    } else if (isTextDeltaMessage(msg)) {
      text += msg.textDelta;
    } else if (isFinishMessage(msg)) {
      if (msg.conversationId) conversationId = msg.conversationId;
      const finalText = extractTextFromResult(msg.result);
      if (finalText) text = finalText;
      item = extractItemFromResult(msg.result);
      if (msg.result?.conversation_id) {
        conversationId = msg.result.conversation_id;
      }
    } else if (isErrorMessage(msg)) {
      error = msg.error;
    } else if (isProgressMessage(msg) && msg.conversationId) {
      conversationId = msg.conversationId;
    }
  }

  return { text: text.trim(), conversationId, item, error };
}
