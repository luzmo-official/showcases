...
## Luzmo APIs called from the browser

| Endpoint | Purpose |
|----------|---------|
| `POST /0.1.0/data` | Source-table rows, KPI aggregations, row counts |
| `POST /0.1.0/securable` | Dataset metadata (columns, row count source) |
| `POST /0.1.0/aichart` | Natural-language chart generation |

All requests include the embed key/token in the body — no Authorization header, no backend proxy.

## Rotating the embed token

The embed is valid for **1 year** (see `ROUTEDATA_LUZMO_EMBED_EXPIRY`). To mint a replacement, run this once in a Node REPL with your Luzmo API key/token (keep the API key **off** the client):

```js
import Luzmo from '@luzmo/nodejs-sdk';
const client = new Luzmo({ api_key: '...', api_token: '...', host: 'https://api.luzmo.com' });
const authorization = await client.create('authorization', {
  type: 'embed',
  username: 'routedata-demo-user',
  name: 'RouteData',
  suborganization: 'your-suborg-slug',
  role: 'designer',
  expiry: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString(),
  inactivity_interval: 365 * 24 * 60 * 60,
  access: { datasets: [{ id: '<dataset-id>', rights: 'use' }] },
});
console.log(authorization.id, authorization.token);
```

Paste the `id` / `token` / expiry into `.env.local`.

## Further Reading

See [AGENTS.md](./AGENTS.md) for architecture rules, module structure, and implementation guidelines.
